...
- 通过主题 ID 或详情页网址，直接从 [CodexThemes](https://codexthemes.ai/zh) 安装兼容的 `.codex-theme` 主题包
...
主题库现在包含两个可搜索视图：**本地主题** 按名称、ID 或描述筛选已安装主题；进入 **Market** 后，即使尚未输入搜索内容，也会立即加载完整 CodexThemes 目录，之后可以再按风格或主题筛选。所有条目都可以选择并在右侧大图预览；所有 `kind=theme` 的正式主题都能安装到本地，包括受限大小的手动 ZIP 和无 artwork 的纯 CSS 主题。点击 **安装** 只加入主题库；点击 **安装并应用** 会安装后立即应用到 Codex，执行运行时健康检查，并在验证失败时自动回滚。`+` 菜单仍支持粘贴 `portal-panic` 这类主题 ID 或详情页网址直接安装。`kind=skin` 是用于创建主题的设计参考，本身没有主题包，因此保留搜索与预览，但不会伪装成可安装主题。
