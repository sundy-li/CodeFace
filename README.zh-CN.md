...
CodeFace 独立 App 是一个精简控制器，用于打开主题管理、修改 CodeFace 自身的语言和明暗配色、重启 Codex 或关闭 Codex。
...
完整主题库位于 Codex Settings 的 **外观** 下方。你可以浏览本地与 Market 主题、自动加载高清效果预览、应用或修改主题，并通过精简的“更多”菜单使用其他操作。

![Codex Settings 中的 CodeFace 主题管理](resources/CodeFace-Settings.png)
...
- 在内置 [CodexThemes](https://codexthemes.ai/zh) Market 中搜索、预览、安装并应用兼容的 `.codex-theme` 主题包
- 通过增量 DOM 兼容、稳定的 suggestion 和图片焦点避让，保留市场主题的预期效果
- 在 CodeFace 内搜索并预览市场主题，可选择安装或安装并应用，同时支持检查更新、备份回滚与导出
- 根据真实 Codex DOM 验证应用结果，并在主题不健康时自动回滚
- 直接编辑 `theme.json` 和 `codeface.css`，CSS 编辑器支持语法高亮
- 在 Codex Settings 的 **外观** 下方直接打开 CodeFace，无需离开 Codex 即可管理本地与市场主题、编辑源码、导入导出主题包，并在原生外观与自定义主题之间切换
...
- 首页主题预览和四个原生快捷建议按钮
- 从管理器关闭或重启 Codex
- 从独立控制器检查 GitHub Latest Release；通过校验的新版本会自动下载、安装并重启
- English 与简体中文界面
- 可单独配置 CodeFace 独立窗口的语言与明暗配色
- CodeFace 独立窗口只保留主题设置、自身两项偏好、Codex 生命周期控制和 CodeFace 更新
...
主题库现在包含两个可搜索视图：**本地主题** 按名称、ID 或描述筛选已安装主题；进入 **Market** 后，即使尚未输入搜索内容，也会立即加载完整 CodexThemes 目录，之后可以再按风格或主题筛选。所有条目都可以选择并在右侧大图预览；所有 `kind=theme` 的正式主题都能安装到本地，包括受限大小的手动 ZIP 和无 artwork 的纯 CSS 主题。点击 **安装** 只加入主题库；点击 **安装并应用** 会安装后立即应用到 Codex，执行运行时健康检查，并在验证失败时自动回滚。`kind=skin` 是用于创建主题的设计参考，本身没有主题包，因此保留搜索与预览，但不会伪装成可安装主题。

Market API 属于外部数据源：可选文本元数据即使缺失或为 `null`，也不会阻止目录加载。缺少有效主题 ID 或名称的异常条目会被单独忽略，不会再因为一条错误数据导致整个 Market 页面加载失败。若 CodexThemes 条目只有效果参考图、没有可下载主题包，CodeFace 会明确标记为 **仅供参考**，不再显示误导性的安装操作。
