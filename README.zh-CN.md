...
主题库现在包含两个可搜索视图：**本地主题** 按名称、ID 或描述筛选已安装主题；进入 **主题市场** 时会默认加载全部 CodexThemes 主题，也可以再按风格或主题搜索筛选。选择市场结果后，右侧主区域会加载大图预览，此时不会安装主题。点击 **安装** 只加入主题库；点击 **安装并应用** 会安装后立即应用到 Codex，执行运行时健康检查，并在验证失败时自动回滚。`+` 菜单仍支持粘贴 `portal-panic` 这类主题 ID 或 `https://codexthemes.ai/themes/<id>` 详情页网址直接安装。标记为皮肤或不可安装压缩包的条目不支持此流程。
