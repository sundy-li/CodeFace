...
Market responses are treated as external data: optional text metadata may be missing or `null` without preventing the catalog from loading. Entries without a usable theme ID or name are skipped individually, so one malformed listing cannot make the entire Market view fail.
